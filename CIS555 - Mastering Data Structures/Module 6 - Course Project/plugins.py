    ...
    for row in range(height):
        for col in range(width):
            ...
            if sepia == False:
                ...
                new_blue = int(brightness * 0.4)
                new_green = int(brightness * 0.6)
...
def flip(image,vertical=False):
    ...
    if vertical == False:
        ...
        for row in range(height//2):
            for col in range(width):
                image[row][col], image[-row-1][col] = image[-row-1][col], image[row][col]
...
# Use this function call to manipulate the images.
#python3 pictool.py display images/debug.png
