...
def str_to_time(timestamp,tz=None):
    ...
    try:
        ptimestamp = parse(timestamp)

        if ptimestamp.tzinfo is not None:
            return ptimestamp

        elif tz is not None and ptimestamp.tzinfo is None:
            if type(tz) == str:
                zone = pytz.timezone(tz)
                newtimestamp = zone.localize(ptimestamp)
                return newtimestamp

            else:
                ts = ptimestamp.replace(tzinfo=tz)
                return ts
...
def daytime(time,daycycle):
    ...
    try:
        ...
        test_time = time.strftime('%H:%M')
        print(test_time)

        print(sunrise < test_time)

        print(sunset > test_time)

        print(test_time < sunset)

        if sunrise < test_time:
            if test_time < sunset:
                return True
