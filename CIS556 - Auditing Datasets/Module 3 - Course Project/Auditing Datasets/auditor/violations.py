    ...
    try:
        if winds['units'] == 'MPS':
            if winds['gusts'] > winds['speed']:
                KT = winds['gusts'] * 1.94384
                CW = winds['crosswind']

                if KT < maxwind or CW < maxcross:
                    return True
                elif KT > maxwind or CW > maxcross:
                    return True
                else:
                    return False

            else:
                KT = winds['speed'] * 1.94384
                CW = winds['crosswind']
...
def bad_ceiling(ceiling,minimum):
    ...
    for x in ceiling:
        ...
        elif x['type'] == 'indefinite ceiling':
            if x['height'] < minimum:
                return True
            else:
                return False
...
def get_weather_report(takeoff,weather):
    ...
    takeoff_time = datetime.isoformat(takeoff)

    takeoff_time = takeoff_time[:14] + '00' + takeoff_time[16:]
    
    new_time = utils.str_to_time(takeoff_time)

    if takeoff_time in weather:
        return weather[takeoff_time]

    else:
        closest = datetime(1901,1,1,tzinfo=timezone.utc)
        for report in weather:
            
            comp = utils.str_to_time(report)
            if comp < new_time:
                
                if comp > closest:
                    closest = comp
        if closest == datetime(1901,1,1,tzinfo=timezone.utc):
            return None

        else:
            t = datetime.isoformat(closest)
            return weather[t]
...
def get_weather_violation(weather,minimums):
    ...
    ceiling = weather['sky']
    #print(ceiling)
    ceiling_check = bad_ceiling(ceiling,minimums[0])
    #print(ceiling_check)

    visibility = weather['visibility']
    #print(visibility)
    vis_check = bad_visibility(visibility, minimums[1])
    #print(vis_check)

    winds = weather['wind']
    wind_check = bad_winds(winds,minimums[2],minimums[3])
    #print(wind_check)

    # print()
    # print(ceiling, minimums[0])
    # print(visibility, minimums[1])
    # print(winds,minimums[2],minimums[3])
    # print()

    if ceiling_check and not vis_check and not wind_check:
        return 'Ceiling'

    elif vis_check and not ceiling_check and not wind_check:
        return 'Visibility'

    elif wind_check and not vis_check and not ceiling_check:
        return 'Winds'

    elif not wind_check and not vis_check and not ceiling_check:
        return ''
...
def list_weather_violations(directory):
    ...
    """
    fpath  = os.path.join(directory,WEATHER)

    # Load in all of the files
    weather = utils.read_json(fpath)

    fpath  = os.path.join(directory,LESSONS)
    lessons = utils.read_csv(fpath)[1:]

    fpath  = os.path.join(directory,STUDENTS)
    students = utils.read_csv(fpath)[1:]

    fpath  = os.path.join(directory,DAYCYCLE)
    daycyle = utils.read_json(fpath)

    fpath  = os.path.join(directory,MINIMUMS)
    minimums = utils.read_csv(fpath)[1:]
    ...
    for lesson in lessons:
        ...
        takeoff_date = utils.str_to_time(lesson[3][:11])
        ...
        creds = pilots.get_certification(takeoff_date,student)
        area = lesson[6]
        instructed = True if lesson[2] else False
        ...
        day_time = utils.daytime(takeoff_time,daycyle)

        pilot_min = pilots.get_minimums(creds,area,instructed,vfr,day_time,minimums)

        report = get_weather_report(takeoff_time,weather)

        violation = get_weather_violation(report,pilot_min)
        
        if violation != '':
            lesson.append(violation)
            violations.append(lesson)
    return violations