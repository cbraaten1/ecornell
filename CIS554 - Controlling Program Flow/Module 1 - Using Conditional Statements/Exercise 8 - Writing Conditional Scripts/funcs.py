...
def first_vowel(s):
    ...
    if introcs.find_str(s,'a') != -1:
        str_length = introcs.find_str(s,'a')

    if introcs.find_str(s,'e') != -1 and introcs.find_str(s,'e') < str_length:
        str_length = introcs.find_str(s,'e')

    if introcs.find_str(s,'i') != -1 and introcs.find_str(s,'i') < str_length:
        str_length = introcs.find_str(s,'i')

    if introcs.find_str(s,'o') != -1 and introcs.find_str(s,'o') < str_length:
        str_length = introcs.find_str(s,'o')

    if introcs.find_str(s,'u') != -1 and introcs.find_str(s,'u') < str_length:
        str_length = introcs.find_str(s,'u')

    if introcs.find_str(s,'y',1) != -1 and introcs.find_str(s,'y') < str_length:
        str_length = introcs.find_str(s,'y',1)

    return -1 if len(s) == str_length else str_length
...
def pigify(s):
    ...
    first_vowel_index = first_vowel(s)
